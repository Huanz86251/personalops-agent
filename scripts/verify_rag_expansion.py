    ...
    with trace_span('Local RAG Acceptance', input_value={'paid_model_requests':0,'fixture':'authored documentation; not AppWorld score'}) as root_span:
        ...
        with retrieval_scope(hub, 'test', stamp):
            ...
            for role in ('General Agent','Code Worker','Code Reviewer','Web Agent'):
                with trace_span(role, kind='agent'):
                    update = await KnowledgeMiddleware(role).abefore_agent({'messages':[{'role':'user','content':'How does SUM add numbers?'}]}, None, {'configurable':{'thread_id':role}})
                    role_checks[role] = bool(update.get('knowledge_context'))
                    agent_key = update['knowledge_agent_key']
                    refs = grants(hub,stamp,agent_key)
                    if not refs: raise AssertionError(role+' missing reference')
                    rt = SimpleNamespace(state=update,config={'configurable':{'thread_id':role}})
                    page = await read_knowledge.coroutine(next(iter(refs)),rt,2,0)
                    if page['status'] != 'ok' or page['tokens'] > 2000: raise AssertionError(page)
        ...
        result['role_auto_read'] = role_checks
        result['passed'] = all(role_checks.values()) and bool(scheduler and code and not unrelated and result['manual_statuses']==['ok','ok','budget_exhausted'])
